...
- Поддерживает сервер Голос без OpenAI, премиум-режим Голос и прямой OpenAI-режим с личным ключом пользователя.
...
## 6. Как переводить текст с экрана

По умолчанию экранный переводчик включён на клавише `F9`:

1. Нажмите `F9`.
2. Выделите мышью область с текстом. `Esc` отменяет выбор.
3. Дождитесь распознавания и перевода.
4. В окне результата нажмите `Копировать перевод`.

В OpenAI отправляется только выделенная область экрана. Снимки, исходный текст и перевод не сохраняются в файлы или диагностику.

Настройки находятся во вкладке `Зрение`. Экономичный режим по умолчанию использует `gpt-5-nano`. Для мелкого текста или сложного фона можно выбрать `gpt-5.6-luna`.
...
Для экранного перевода нужен интернет и личный `OPENAI_API_KEY`. Голосовой локальный режим при этом продолжает работать без отправки аудио в интернет.

## 7. Как выбрать локальный режим
...
- `server` - отправляет аудио на сервер Голос, где оно распознаётся локальной серверной моделью без OpenAI.
- `openai` - отправляет аудио в OpenAI API и обычно лучше справляется со сложной диктовкой, если задан `OPENAI_API_KEY`.
- `premium` - отправляет аудио на сервер Голос Премиум, сервер использует лучший доступный режим и списывает минуты с баланса.
...
Если локальное распознавание даёт много ошибок, можно попробовать профиль `server`, OpenAI-профиль или профиль `premium`.
...
Есть интернет-режимы:

- `Профиль: server` - аудио отправляется на сервер Голос и распознаётся серверной локальной моделью без OpenAI.
...
- `Профиль: premium` - аудио отправляется на сервер Голос, сервер распознаёт его через OpenAI и списывает минуты с премиум-ключа.
...
## 8. Как включить сервер Голос без OpenAI

Этот режим требует интернет, но не требует личный OpenAI API-ключ и не списывает премиум-баланс.

```powershell
.\.venv\Scripts\python.exe -m voice_input.app --set-profile server
```

Технически это соответствует настройке:

```yaml
recognition_profile: "server"
...
Если сервер недоступен, приложение пробует локальный `local_fast`.

## 9. Как включить OpenAI-режим
...
## 10. Как включить Голос Премиум

Откройте меню трея, выберите `Открыть настройки`, перейдите на вкладку `Распознавание`.
Выберите профиль `Премиум Голос - через наш сервер`.

Затем откройте вкладку `Аккаунт`, войдите или зарегистрируйтесь по email и паролю.
На этой же вкладке отображается баланс минут и кнопка `Пополнить`.
Оплата открывается на стороне платёжной системы; данные карт в программе не вводятся и не хранятся.

Сессионный токен аккаунта хранится локально в `.env` как `GOLOS_ACCOUNT_TOKEN`.
В профиле `Премиум Голос - через наш сервер` пользовательский OpenAI API-ключ не нужен.

Технически это соответствует настройке:

```yaml
recognition_profile: "premium"
...
Старый ручной `GOLOS_PREMIUM_KEY` остаётся совместимым для админского резервного сценария, но обычному пользователю ключ вставлять не нужно.
Если аккаунт-токен или премиум-ключ не найден, приложение не запускает серверное распознавание и пробует локальный `local_fast`.
...
## 12. Как добавить автозапуск Windows
...
В окне настроек на вкладке `Диагностика` можно указать сервер поддержки и нажать `Отправить диагностику`.
Если сервер не указан, программа откроет GitHub Issue и папку с ZIP-архивом для ручной отправки.
Если есть премиум-ключ, диагностика может быть отправлена на сервер Голос по этому ключу без отдельного служебного токена.
