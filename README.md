...
Откройте меню трея, выберите `Открыть настройки`, перейдите на вкладку `Распознавание`.
В поле `Премиум-ключ Голос` вставьте выданный ключ, нажмите `Сохранить`, затем `Проверить баланс`.

Премиум-ключ хранится локально в `.env` как `GOLOS_PREMIUM_KEY`.
...
Если премиум-ключ не найден, приложение не запускает серверное распознавание и пробует локальный `local_fast`.
