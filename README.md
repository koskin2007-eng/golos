...
В собранной EXE-версии обновление можно запустить из окна настроек на вкладке `Обновления`: программа скачает ZIP, проверит SHA256, закроет текущий процесс, заменит файлы и запустит новую версию. Пользовательские файлы `.env`, `config.yaml`, `logs/`, `diagnostics/`, `temp/`, `models/` и `updates/` при замене сохраняются.
...
Перезапустить приложение можно из меню значка в трее: `Перезапустить Голос`.
Такая же кнопка есть в окне `Открыть настройки`.
В этом же окне есть кнопка `Создать ярлыки Windows`, чтобы добавить Голос в меню «Пуск» и обновить автозапуск.
