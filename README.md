...
Настоящий ключ впишите после знака равно только в локальном `.env`.
...
4. Добавьте ярлык на `dist\VoiceInput\VoiceInput.exe`.
...
- Если активное окно запущено от администратора, запустите VoiceInput тоже от администратора.
...
У приложения есть защита от дублей. Если VoiceInput уже запущен в трее, новый запуск сразу завершится и напишет в консоль:
...
- Для некоторых приложений может потребоваться запуск VoiceInput от администратора.
...
В меню трея есть пункт `Collect diagnostics`. Он создаёт zip-архив в папке:
